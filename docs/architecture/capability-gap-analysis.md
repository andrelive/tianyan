...
| **G5** | P1 | 定时任务跨运行状态 | Devin Scheduled Devins "**carries state between runs**"（任务读/写自己的笔记） | scheduler 任务独立运行，无任务级持久状态 | 任务作用域 notes 文件（VFS `tianyan://tasks/{id}/`），RuleTask 运行时可读上次摘要，跨轮上下文延续 |
| **G6** | P1 | 结构化 Trace/span 树 | trace-grading 已成核心原语（OpenAI evals、LangSmith run/trace/thread）；benchmark gaming 审计依赖可回放 trace | 检索轨迹 FIFO 500 + 内存态 AgentMetrics | span 树落库（工具调用嵌套 → 轮 → 会话），支持事后回放与坏例分析；为 CI 评测闭环（G9）铺路 |
| **G7** | P1 | MCP 现代化 | **2026-07-28 规范重大变更**：stateless 核心 + AWS Tasks 扩展；远程服务器 **17% 死端点** | MCP 客户端仅 **stdio** 传输 | 增加 streamable HTTP 传输（远程 MCP 服务器——生态主流形态）；接入服务器健康检查；跟踪 Tasks 扩展 |
