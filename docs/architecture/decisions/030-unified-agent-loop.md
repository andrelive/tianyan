...
## 后续演进（2026-09-13）：工具表稳定性契约与会话级变化检测

**背景**：工具定义（`tools` 数组）是提示词前缀的一部分。两处不稳定性会白白
打掉前缀缓存：① 动态工具表用 `HashMap`（遍历顺序不确定，配 MCP 后同一工具集
可能每次请求产生不同字节序列）；② 角色清单动态追加进工具描述（角色演化即
触发，见 ADR-016 后续演进）。

**演进**：

1. **稳定性契约**（`ToolRegistry::definitions`）：内置按注册顺序 + 动态工具
   **注册顺序追加末尾**（`Vec` 取代 `HashMap`；新工具**不移位**已有工具——
   "追加末尾"使新增工具的缓存失效范围最小）；按名去重（重名注册跳过）。
   回归测试：连续两次取定义**逐字节一致**（反向验证：遍历反转 / 内容漂移必红）。
2. **会话级变化检测**（`SessionState.toolset_fingerprint`）：真用户轮
   （`TurnOptions.do_toolset_check`，仅 `process_message` / `process_message_stream`
   两处开启；唤醒轮不经 `run_agent_turn`，天然不检查）在请求前比对工具表指纹
   （`ToolRegistry::toolset_fingerprint()`，确定性哈希）——变化（MCP 增删 /
   配置热重载）→ **主动压缩一次**重建前缀：变化立即可见 + 缓存失效与压缩
   合并到同一轮；首轮（新会话/重启）只记基线不触发。
3. 角色清单移出工具描述（见 ADR-016 后续演进）。

**取舍记录**：请求侧工具定义**每轮现取**全局注册表（**不冻结**会话快照）——
冻结需把会话维度穿进请求组装链（`run_turns` → step 闭包 → `prepare_request`
签名改造），而"顺序/内容确定 + 变化检测触发压缩"已**等价**达成"会话内稳定 +
变化立即可见 + 缓存失效可控"；故不引入冻结层（少一层结构与改造面）。
