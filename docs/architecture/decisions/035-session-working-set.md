...
**日期**: 2026-09-16
**状态**: ✅ 已采纳（**全部阶段 + 写侧收口 + 段化已实施** 2026-09-16）
...
> **✅ 段化已实施（2026-09-16）**：快照键先统一到消息 ID（`ced9ee2`，捕获端不再
> 依赖"内存态条数"），随后工作集引入 `start_seq`（段起点 = 最近压缩点）——
> `rebuild`/`rewrite` 只物化段；`append` 的 seq 改由库尾推进（不数段内条数）；
> `delta_since` 按段坐标切片（ctx 早于段起点 → 整段）；**压缩后收缩**
> （`reshape_after_compression`：段起点前移，否则段随会话无限增长）；消费水位随收缩
> 推进。测试：段化重建 / append seq / delta 边界 / 压缩收缩 4 条 + 判别力实证
> （注入"保留全链"→ 段化测试红）。ADR-027 的「完整链」仍是**存储层**契约（回退/
> 快照按库读），工作集作为组装缓存只持其后缀——两者职责不同，不冲突。
...
## 实施修订：页边界对齐用户消息（2026-09-17）

**问题（实测）**：50 条硬切的分页边界有约 50% 概率切在「工具调用 → 其
结果」相邻对之间（全链 4633 条实测：92 个页界 **46 命中**）。含结果的页
缺调用 → 结果渲染为孤立"工具结果"卡；含调用的页缺结果 → 调用卡
`result=null` → 前端渲染"运行中"转圈（对已完成的历史工具，误导）。
成因：工具结果的跨消息合并作用域 = **单页批窗口**（T0-7 同类症状的
"分页版"；一致性回归清单 #8"拼接==全量"在**转换可见形态**口径下不成立）。

**修订（页边界对齐 user）**：

- 以 50 条为基准取窗口 → 页从「**窗口内由远及近第一条 `user`**」起（截取）；
- 窗口内没有 → **直接向前找最近一条更早的 `user`**（一次 `MAX(seq)` 反向
  查询，不按批扩展、不设人为上限）；到链头仍无 → 整段（链头..窗口尾）兜底；
- 页首恒为用户消息 → 页边界切缝形态恒为 `(X | user)`，调用与结果不会被
  切开；页长由「轮间隔」决定（1..N）；
- 落点：`SessionStore::last_user_seq_before`（SQL）+ `SessionManager` trait
  默认实现（全量加载）/持久化覆盖 + `get_messages_page` 对齐逻辑；
  **前端零改动**（游标语义不变）。

**全链模拟（4633 条）**：跨页「调用|结果」对 **46 处 → 0**；页首=user
**56/56**；拼接覆盖 **100%**（无缺无重）；页大小 1..446（avg 83）。

**测试**：store 单测（`test_last_user_seq_before`）+ server 三个对齐测试
（窗口内 / 跨窗口查找 / 窗口头不缩水 / 链头兜底；"调用|结果"同页的直接
判据 = 合并后 `result.is_some()`）。
