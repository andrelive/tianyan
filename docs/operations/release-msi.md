...
| --- | --- | --- | --- |
| `tauri/tauri.conf.json` → `version` | **`0.3.17`** | ✅ **是**（唯一权威） | **MSI 文件名与 ProductVersion 由它决定**（安装后在「应用和功能」可见） |
| `Cargo.toml` → `[workspace.package] version` | `0.3.17` | ❌ 否（但决定 **exe 文件属性**） | 各 crate 版本 + 编译产物的 `FileVersion`/`ProductVersion`。**0.3.16 起与产品版本统一**：此前为 `0.1.0`，导致 exe 属性里显示旧版本（MSI 名不受影响） |
| `gui-vite/package.json` → `version` | `0.1.0` | ❌ 否 | 前端包版本（name `tianyan-gui`），不参与 MSI |
| `docs/release/RELEASE_NOTES.md` | `0.3.17` | ❌ 否 | 发布说明（人工维护） |
| `CHANGELOG.md` | `0.3.17` | ❌ 否 | 变更日志（人工维护） |
...
- 语言取 `tauri.conf.json.bundle.windows.wix.language = ["zh-CN","en-US"]` → **一次构建产出两个 MSI**：
  - `target/release/bundle/msi/Tianyan_0.3.17_x64_zh-CN.msi`
  - `target/release/bundle/msi/Tianyan_0.3.17_x64_en-US.msi`
...
- [ ] **版本号**：应用内“关于/更新检查”显示 `0.3.17`（与 MSI 版本一致）。
