...
- **模型切换同名歧义（跨 provider 无法精确切换）**：`switch_model` 按模型名在**第一个**已启用提供商中查找——同名模型（如 opencode 与 ollama 都挂载 `glm-5.3-flash`）时列表两项点击结果完全相同，永远切不到第二家的同名模型。修法：`SwitchModelRequest` 新增可选 `provider` 字段（缺省保持历史语义），指定 provider 时精确匹配；前端切换请求携带 provider、`selectedModel` 升级为 provider+model 复合引用（选中高亮 / 思考档位匹配同源），顺带修掉同名项高亮双亮与列表 key 冲突
- **输入草稿切页面 / 切会话丢失**：输入框文字与待发图片是组件本地 state——路由切换（组件卸载）或切换会话即丢。修法：草稿按会话键上提前端 store（`inputDrafts`；未创建会话归 PENDING 槽），切换页面/会话恢复、发送后清空、图片（data URL）一并缓存；异步读图用函数式更新合成（不覆盖等待期间输入的文字）
...
- **回退时回填输入草稿（编辑重发）**：回退到某条用户消息后，该消息（文本 + 图片）自动回填当前会话的输入框——直接编辑重发，不用重新打字；草稿显式写回发起会话（请求期间切走不污染其他会话）
...
### Fixed
- **工具目录门禁命令漂移（CI 必红 → tag 推送后不出包）**：0.5.8 把工具目录生成器从 core 迁到 server 侧（组件工具也要进目录），同步了 `scripts/gen-tool-catalog.ps1`，但 **CI（`quality.yml`）里内联的旧命令未改**（`cargo run -q -p tianyan-core --example tool_catalog -- --check`）→ tag `v0.5.9` 的 Quality 门禁报 `no example target named tool_catalog in tianyan-core` 必红 → `release.yml` 的 build 因 `needs: quality` 被跳过（**发布不出包**）。修法不是"再记住同步一处"，而是**命令单一来源**：新增仓库级 `.cargo/config.toml` 的 alias（`catalog-check` / `catalog-gen`）承载命令本体（生成器在 server 侧），CI 步骤、`gen-tool-catalog.ps1`、`wsl-ci-parity.sh`、文档一律调别名——换生成器位置只改一处。顺带修 `gen-tool-catalog.ps1` 的 `$catalogPath` 拼接（单段 `"docs\architecture\..."` 在 Linux pwsh 下会得到坏路径 → 分段 `Join-Path`）。判别力实证：注入目录漂移后 `cargo catalog-check` 退 1、`gen-tool-catalog.ps1 -Check` 退 1（正常态均 0）。文档同步：`release-msi.md`（门禁表 + quality 触发描述已过时）+ ADR-037 生成器路径引用
...
- **VFS sqlite 后端静默吞错（`exists` 折叠 SQL 故障、`list_directory` 藏坏行）（P1-37）**：`exists` 用 `unwrap_or(0)`——SQL 故障返回"不存在"（上层据此误判条目缺失并跳过/重建）；`list_directory` 用 `filter_map(|r| r.ok().flatten())`——行级损坏彻底隐身（条目"凭空消失"无线索）。修法：`exists` 错误上抛（COUNT 正常路径不返回空行）；`list_directory` 记录跳过行数并 `warn!`（**行为等价、可观测性增强**——该侧无判别测试，如实记录）
- **并行工具结果按完成顺序返回（落库/上下文顺序不稳定，打掉前缀缓存）（P1-31）**：`execute_parallel` 用 `JoinSet::join_next()` 收集 → 结果顺序 = 完成顺序（不确定）——下游按该顺序落库、进上下文、流式推送：历史消息顺序随机，且下一轮请求前缀与上一轮不一致（前缀缓存失效）。修法：按 `calls` 原序回填（HashMap by id）。判别力：`test_execute_parallel_preserves_call_order`（注入"按完成顺序收集"→ 必红：慢调用不再排首）
- **工作集 `append` 自推 seq（绕过写入后与库错位）（P1-34）**：`SessionWorkingSet::append` 用 `last_seq()+1` 自推——库已原子取号（`MAX(seq)+1`）但 `append_message` 不返回；绕过工作集的直写/并发写使自推值与库分配值**错位**（内存水位落后库尾，依赖 `ensure_fresh` 兜底重建）。修法：`append_message`/`append_message_no_fts` 返回真实 `seq`，工作集按返回的 seq 落位。判别力：`test_append_uses_db_allocated_seq_when_bypassed`（注入"自推"→ 必红）
- **快照 GC 在树文件损坏时误删活对象 + 恢复非原子 + 快照写非原子（P1-14）**：① `collect_tree_hashes` 对无法解析/读取的树文件只 warn 跳过——该树引用的对象因"不可达"被判孤儿**删除**（活快照数据丢失）；② `restore_tree` 边写边读对象——写到一半才发现对象缺失，留下半恢复工作区；③ `capture_tree_to` 用裸 `fs::write` 写快照树/缓存（半写即损坏，触发 ①）。修法：① 任一树文件不可解析/读取 → **跳过本轮清扫**（安全优先，待修复后下轮再清）；② 恢复前**全量预校验**对象存在（缺失即中止，工作区未改动）；③ `temp + rename` 原子写（新增 `write_atomic` 单点）。**顺带修复**：`collect_tree_hashes` 把 `{index}.cache.json`（缓存，结构与树不同）当树文件解析——旧行为仅 warn 噪音，保守化后会令 GC 永久跳过；现显式排除。判别力：`test_gc_skips_sweep_when_tree_corrupt` + `test_restore_aborts_before_writing_when_object_missing`（注入修复前口径 → 两条必红）；`test_capture_tree_uses_atomic_write` 为防护断言（原子性不可从外部观测，判别力有限，如实记录）
- **调度器无任务级超时（单任务 hang 拖死全部定时任务）**：扫描循环串行执行任务、`execute_task` 直接 `handler.execute(ctx).await`——某任务 hang（LLM 请求无响应等）会让**整个**定时任务体系停摆（后续 tick 与其它任务全不跑）。修法：任务执行包 `tokio::time::timeout`（默认 `TASK_TIMEOUT_SECS = 1800`，`with_task_timeout_secs` 可覆盖），超时中断该任务、记 `timeout` 语义失败（`last_error` 可追溯）并继续后续任务。判别力：`test_execute_task_times_out_and_records_failure`（注入"超时永不触发"→ 必红：任务 30s 后仍报成功）
- **运行期状态文件非原子写（断电 → 全部任务同时补跑）**：`scheduler_state.json` 用裸 `fs::write`——半写文件被 `load_state` 判为损坏 → 所有任务视为从未执行 → 同时补跑；且损坏文件被静默忽略（诊断样本丢失）。修法：temp + rename 原子替换（Windows 亦为替换语义），损坏时另存 `.corrupt` 保留证据。判别力：`test_state_file_corrupt_quarantined_and_atomic_write`（注入"不另存"→ 必红）
- **工作集并发 `ensure` 双实例（缓存/写收口分叉）**：检查—重建—插入之间无锁（`rebuild` 是 await 点）——并发 `ensure` 各建一个 `SessionWorkingSet`，后者覆盖注册表 → 先返回的实例"脱管"（其后续写入绕过注册表，缓存与库分叉）。修法：以**先入者**为准（`entry().or_insert()`：后到者丢弃自建实例、返回注册表中的那一个）。另修 `sweep_idle` 使用中判定：除"会话锁被持有"外，**工作集实例被外部引用（`strong_count > 1`）也不卸载**（有人 `get` 到 Arc 但未持锁时卸载会造成同类分叉）。判别力：`test_sweep_keeps_externally_referenced_set`（注入"只看锁"→ 必红）；`test_concurrent_ensure_yields_single_instance` 为防护断言——**注入旧口径仍绿**（`join!` 轮询下 `rebuild` 无让出点，旧代码恰好也返回同一实例；真实竞态需多线程 runtime，测试不可稳定复现），如实记录
- **向已删除会话写入静默重建幽灵会话（丢 parent、进主列表、消息可被回忆）**：`append_message` 用 `INSERT OR IGNORE` 隐式重建 `session_meta`——级联删除后仍在跑的子代理 / 流式收尾 / 唤醒轮写入，会凭空造出**根会话**（parent 丢失）并进主列表；消息还进 FTS（`session_recall` 可搜到已删会话内容）。修法：会话必须已显式 `create`（生产路径均已如此：新会话 `resolve_or_create_session`、子会话、定时任务），不存在则返回 `not_found` 拒绝写入。**行为变更**：append 不再隐式创建（测试同步收紧）。判别力：`test_append_to_deleted_session_is_not_found` + 改写的 `test_append_seq_monotonic_per_session`（注入旧口径 → 两条必红）
- **系统/运维路径判定面过窄（归档/评审被重新摘要 + 向量化 → 已退役内容可复活注入）**：`is_operational_path` 只对 Memory 命名空间生效（非 Memory 直接 `false`）——`agent/_archive/learned/*`、`agent/roles/_archive/*`、`agent/learned/archive/*`（软删除与 GC 归档）、`skill/_archive/*`、`skill/_reviews/*.jsonl`（评审记录）全部落网：摘要任务为其生成 L0/L1 + 写向量，向量回填/对账（`collect_indexable_uris`）也收进索引 → 已退役规则/技能可被 `load_relevant_rules` / 记忆检索命中注入。修法：新增全命名空间判定单点 `system_paths::is_system_path`（记忆运维子域 ∪ 各命名空间归档/评审；前缀匹配抽为共用 `path_starts_with`），三处消费面统一接入——SummaryTask 扫描、`collect_indexable_uris`（backfill + reconcile 共用）、`DualLayerRetriever::fused_search_in_namespace` 结果出口（存量向量兜底）。判别力：判定单测（含防前缀误伤）+ 扫描排除 + 对账跳过 + 检索过滤共 4 项（注入"非 Memory 不判"→ 全部必红）
- **意图推断可把检索范围限到无向量的 Session（"上次/对话/摘要"类 query 静默检索为空）**：`infer_target_scope` 的候选序列与关键词表都含 Session，而会话内容不入向量库（ADR-018）——中文高频 query 命中即得空结果且无日志线索（表现为"记忆/经验突然不生效"）。修法：Session 移出候选与关键词表（会话回忆统一走 FTS5 `session_recall`，ADR-017 决策 6）。判别力：`test_scope_inference_never_targets_session`（注入旧映射 → 必红）
- **事件订阅读取失败被折叠为空快照（前端 replace 清屏且无提示）**：`POST /events/subscribe` 以 `unwrap_or(-1)` / `unwrap_or_default()` 吞掉 `last_seq` / `load_before` 错误——会话存在性检查通过后若读取失败，推 `messages: []` 快照，前端 `applySnapshot` 为 replace 语义 → 界面消息被清空且无提示。修法：读错误照常上抛（500，不推快照）。正常路径由既有 e2e 回归覆盖；**该条无判别测试**——http 级无法构造"会话存在但存储读取失败"（如实记录于审查档案）
...
### Changed
- **VFS 三层语义重定义：简介 · 目录 · 正文（ADR-001 修订）**：L1 从"同质概览"改为**章节目录**——兑现 ADR-001 原始定位（"内容导航"）；多文件技能 = VFS 目录节点（成员目录消费时现遍历，**零生成**，不引入新实体）。四项落地：① `vfs_list` 条目补 L0 简介（"现遍历成目录"，零新增查询）；② 叶节点 L1 = 结构化目录（`SummaryEngine::generate_overview`：一次 LLM 调用分节 + 每节摘要 + 锚点 → **程序按标题/锚点定位回填行号**，LLM 不报行号；短内容"直用"保持；**目录为默认输出**——仅代码/脚本/数据表等无法分节的形态降级为精炼概览；长度守卫沿用；解析/定位失败降级）；③ **L2 不再自动加载**（`ContentLoadStrategy`：>0.6 → 简介 + 目录、≤0.6 → 简介；目录渲染 markdown 附行号）；④ `vfs_read` 支持 `offset/limit` 行范围取段（正文默认头部截断 2000 行/50KB + "使用 offset 继续"——顺带收口正文无上限倾泻）。**前端同步**：记忆/知识面板层级标签 L0/L1/L2 → **简介/目录/正文**，L1 目录 JSON 渲染为可读列表（`lib/vfs-index.ts`，与后端 `render_doc_index` 格式对齐——目录不再原样裸露 JSON）。**不破 chunk 禁令**：章节是读取视图（目录条目 + 行号 range），L2 仍为单一完整文本、不独立索引（REJECTED #4 补边界澄清）；schema 与检索机制零改动。判别力：+11 测试；`pipeline` rules/memories 夹具补 L1 即"L2 不自动加载"的行为变更证据
- **`task_status` 列表瘦身（370KB 级全量 dump 治理）**：列表模式（无 `task_id`）对每个可见任务 `serde_json::to_value(t)` **全量序列化**——`result`（子代理完整报告）/ `output_tail` / `log_file` / `result_path` 等重字段全带，真实使用中产生 **369–375KB** 结果（三次调用实测；另有 `execute_command` 443.9KB 历史条目同证"结果侧规模失控"）；而"超大文件"类路径（工作区读取 / VFS 内容，最大 43KB）反而全在安全线内。修法：列表返回**定位用精简条目**（id / kind / status / 描述截断 360 字节 / 时间；失败任务附截断 error；命令任务附 command 截断 + ready）——完整字段只在 `task_id` 详情查询返回；**默认最近 20 条**（`limit` 可调、上限 100；超出附截断 note）。预期 370KB → 2–5KB，**信息不丢**（列表定位 + 详情取全，与 read_file 50KB 截断先例同模式）。判别力：3 条新测试对旧实现 **3/3 必红**（`test_task_status_list_is_compact` / `test_task_status_list_limit_recent_first_and_hint` / `test_task_status_list_command_entry_is_compact`）；`test_task_status_detail_keeps_full_snapshot` 锁定"详情仍全量"契约
