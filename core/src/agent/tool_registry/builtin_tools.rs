...
/// 当前 shell 事实（注入 execute_command 工具描述；ADR-037 单一事实源）。
///
/// 与执行侧 [`crate::executor::shell::ShellSpec`] **同源**——提示词随实际
/// 执行底层（auto 探测 / 用户配置）联动，杜绝"配置了 A 提示还说 B"的漂移。
///
/// **跨平台 freshness 归一化依赖**：`tool_catalog --check` 遍历
/// `executor::shell::HINT_TEMPLATES` 替换为占位再比较（CI 在 Linux、仓库
/// 产物在 Windows 生成，不归一化则门禁跨平台恒红）。
fn shell_platform_hint() -> String {
    crate::executor::shell::current_hint()
}
...
fn def_apply_patch(name: &'static str) -> ToolDefinition {
    ToolDefinition::function(FunctionDefinition::from_schema::<ApplyPatchParams>(
        name,
        "对一个或多个文件做修改的**主力编辑工具**。格式：`*** Update File: <路径>` 后直接跟 `-`（删除）/ `+`（新增）/ 空格（上下文）行，只写要改的行 + 少量上下文，不要复现整个文件。`@@` 块头**可省略**（工具按内容定位，无需行号）。`-`/上下文行须与当前文件内容一致。**块内空行**：整行无字符的空行按格式噪声忽略；若原文该行为空、需作为上下文行，须写成「一个空格」的单独一行（上下文行以空格前缀书写，内容可为空）——写成裸空行会让期望匹配窗口少一行。小改、大改、单文件、多文件均适用；编辑前建议先 read_file 目标区确认当前内容。",
    ))
}
