...
///
/// 依据实测（2026-09-21：416 文件 / 5758 定义 / 4023 个名字）：90% 的名字只定义
/// 1 次，定义 ≥6 次的仅 48 个（1.2%）；而 `name` 出现 1099 次、`path` 1102 次——
/// 它们不是「被频繁引用的符号」，是十几个互不相关的同名 getter/trait 方法，
/// 不剔除会把地图顶层完全淹没（前 20 行全是 `path` / `name`）。
///
/// **阈值取 8 而非 6**（2026-09-21 校准）：`config`（定义 7 次 / 被引用 1028 次）、
/// `uri`（7 / 925）、`error`（7 / 472）都是本仓库核心模块——定义 7 次是
/// 「`mod` 声明 + 模块文件」结构的必然结果，不是「互不相关的同名方法」；
/// 阈值 6 会把它们误判剔除（实测默认地图里三者缺席）。阈值 8 / 10 / 12 / 20 的
/// top-10 实测完全一致（已收敛），故 8 是拐点；真正的通用短名仍由第二级过滤
/// （[`is_generic_method_name`]）兜住。
pub const MAX_SHARED_DEFINITIONS: usize = 8;
