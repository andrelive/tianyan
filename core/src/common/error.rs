...
/// 用于创建带上下文的自定义错误的宏。
///
/// 默认映射到 `TianyanError::Other`。对于更具体的错误类型，请使用 `bail_with!`。
#[macro_export]
macro_rules! bail {
    ($msg:expr) => {
        return Err($crate::common::error::TianyanError::Other($msg.to_string()))
    };
    ($fmt:expr, $($arg:expr),+) => {
        return Err($crate::common::error::TianyanError::Other(format!($fmt, $($arg),+)))
    };
}

/// 用于创建具体类型错误的宏。
///
/// 接受任何可转换为 `TianyanError` 的表达式，保留具体的错误变体。
///
/// # 示例
///
/// ```rust,ignore
/// use tianyan::bail_with;
/// use tianyan::TianyanError;
///
/// fn check_config() -> tianyan::common::error::Result<()> {
///     bail_with!(TianyanError::Config("缺少 API 密钥".to_string()));
/// }
/// ```
#[macro_export]
macro_rules! bail_with {
    ($err:expr) => {
        return Err($crate::common::error::TianyanError::from($err))
    };
}

/// 用于确保条件为真，否则返回错误的宏。
///
/// 默认映射到 `TianyanError::Other`。对于更具体的错误类型，请使用 `ensure_with!`。
#[macro_export]
macro_rules! ensure {
    ($cond:expr, $msg:expr) => {
        if !($cond) {
...
/// 用于确保条件为真，否则返回具体类型错误的宏。
///
/// # 示例
///
/// ```rust,ignore
/// use tianyan::ensure_with;
/// use tianyan::TianyanError;
///
/// fn check_depth(depth: usize, max_depth: usize) -> tianyan::common::error::Result<()> {
///     ensure_with!(depth < max_depth, TianyanError::Planning("递归深度超限".to_string()));
///     Ok(())
/// }
/// ```
#[macro_export]
