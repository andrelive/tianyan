 ...
 */
function MigrationDialog({ currentDir, onClose }: { currentDir: string; onClose: () => void }) {
    ...
    pollTimer.current = setInterval(async () => {
      try {
        ...
        if (resp.ok) {
          ...
          let gotDir = '';
          try {
            const cfgResp = await fetch(`${root}/api/v1/config`, {
              signal: AbortSignal.timeout(5000),
            });
            if (cfgResp.ok) {
              const body = (await cfgResp.json()) as {
                config?: { storage?: { data_dir?: string } };
              };
              const norm = (s: string) =>
                s.trim().replace(/\//g, '\\').replace(/\\+$/, '').toLowerCase();
          ...
          <>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-[var(--color-text-tertiary)]">
                确认后系统会先校验目标目录为空（避免其他文件影响），随后自动完成： 关闭数据库 →
                移动数据 → 更新配置 → 重启应用。搬迁期间应用会短暂关闭。
              </p>
            </div>
        ...
        <FieldRow label="向量维度" description="向量嵌入的维度数，通常取决于使用的模型">
          ...
          >
            ...
            {![384, 512, 768, 1024, 1536, 2048, 3072].includes(config.vector_dimension) && (
              <option value={config.vector_dimension}>{config.vector_dimension}（当前值）</option>
            )}
          </select>
        </FieldRow>
